<template>
  <el-dialog :title="title" :visible.sync="visible" width="900px" append-to-body :close-on-click-modal="false">
    <el-form ref="form" :model="form" :rules="rules" label-width="100px">
      ...
      <el-row :gutter="20">
        ...
        <el-col :span="12">
          <el-form-item label="实验编号" prop="expCode">
            <el-input v-model="form.expCode" placeholder="请输入实验编号" />
          </el-form-item>
        </el-col>
      </el-row>
      <el-row :gutter="20">
      ...
      <!-- 模板选择（仅新增时显示） -->
      <el-form-item v-if="!form.expId" label="步骤来源">
        <el-radio-group v-model="temSource" @change="handleTemSourceChange">
          <el-radio label="sop">SOP模板</el-radio>
          <el-radio label="tem">实验模板</el-radio>
          <el-radio label="custom">自定义</el-radio>
        </el-radio-group>
      </el-form-item>

      <el-form-item v-if="temSource === 'sop' && !form.expId" label="SOP模板">
        <div style="display: flex; gap: 10px; align-items: center;">
          <el-input v-model="sopTemName" placeholder="请选择SOP模板" readonly style="flex: 1;" />
          <el-button type="primary" size="mini" @click="openSopTemDialog">选择模板</el-button>
          <el-button v-if="sopTemId" type="text" size="mini" style="color: #f56c6c;" @click="clearSopTem">清除</el-button>
        </div>
      </el-form-item>

      <el-form-item v-if="temSource === 'tem' && !form.expId" label="实验模板">
        <div style="display: flex; gap: 10px; align-items: center;">
          <el-input v-model="expTemName" placeholder="请选择实验模板" readonly style="flex: 1;" />
          <el-button type="primary" size="mini" @click="openExpTemDialog">选择模板</el-button>
          <el-button v-if="expTemId" type="text" size="mini" style="color: #f56c6c;" @click="clearExpTem">清除</el-button>
        </div>
      </el-form-item>
    ...
    <el-dialog title="选择SOP模板" :visible.sync="sopTemDialogVisible" width="750px" append-to-body :close-on-click-modal="false">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="confirmSopTemSelect">确 定</el-button>
        <el-button @click="sopTemDialogVisible = false">取 消</el-button>
      </div>
    </el-dialog>

    <!-- 实验模板选择弹窗 -->
    <el-dialog title="选择实验模板" :visible.sync="expTemDialogVisible" width="650px" append-to-body :close-on-click-modal="false">
      <el-table ref="expTemTable" :data="expTemTableList" size="small" @row-click="handleExpTemRowClick" highlight-current-row height="400">
        <el-table-column label="模板编号" prop="temCode" width="120" />
        <el-table-column label="模板名称" prop="temName" min-width="150" />
      </el-table>
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="confirmExpTemSelect">确 定</el-button>
...
export default {
  ...
  data() {
    return {
      ...
      expTemName: '',
      // SOP模板弹窗
      sopTemDialogVisible: false,
      sopCatalogTree: [],
      sopTemTableList: [],
      currentSopTemId: null,
      // 实验模板弹窗
      expTemDialogVisible: false,
      expTemTableList: [],
      currentExpTemId: null,
      // 当前操作
      ...
      rules: {
        expName: [{ required: true, message: '实验名称不能为空', trigger: 'blur' }],
        expCode: [{ required: true, message: '实验编号不能为空', trigger: 'blur' }]
      }
    }
  },
  created() {
  ...
  methods: {
    ...
    open(row) {
      ...
      if (row && row.expId) {
        this.title = '编辑实验'
        getExp(row.expId).then(response => {
          const data = response.data
          const stepList = response.stepList || []
          this.form.expId = data.expId
          ...
          if (data.planStartTime && data.planEndTime) {
            this.dateRange = [data.planStartTime, data.planEndTime]
          }
    ...
    reset() {
      ...
      this.dateRange = []
      this.temSource = 'custom'
      this.sopTemId = undefined
      this.sopTemName = ''
      this.expTemId = undefined
      this.expTemName = ''
    ...
    // SOP模板弹窗
    openSopTemDialog() {
      this.sopTemDialogVisible = true
      this.currentSopTemId = null
      this.sopTemTableList = []
      catalogTree().then(response => {
        this.sopCatalogTree = response.data || []
      })
    },
    handleSopCatalogClick(data) {
      listSopTem({ temCatalogId: data.id, pageNum: 1, pageSize: 1000 }).then(response => {
        this.sopTemTableList = response.rows || []
      })
    },
    handleSopTemRowClick(row) {
      this.currentSopTemId = row.temId
      this.$refs.sopTemTable && this.$refs.sopTemTable.setCurrentRow(row)
    },
    confirmSopTemSelect() {
      if (!this.currentSopTemId) {
        this.$message.warning('请选择一个SOP模板')
        return
      }
      const selected = this.sopTemTableList.find(item => item.temId === this.currentSopTemId)
      this.sopTemId = this.currentSopTemId
      this.sopTemName = selected ? selected.temName : ''
      this.sopTemDialogVisible = false
      getSopTem(this.sopTemId).then(response => {
        const stepList = response.stepList || []
        this.form.stepList = this.transformStepList(stepList)
      })
    },
    ...
    // 实验模板弹窗
    openExpTemDialog() {
      this.expTemDialogVisible = true
      this.currentExpTemId = null
      listTem({ pageNum: 1, pageSize: 1000 }).then(response => {
        this.expTemTableList = response.rows || []
      })
    },
    handleExpTemRowClick(row) {
      this.currentExpTemId = row.temId
      this.$refs.expTemTable && this.$refs.expTemTable.setCurrentRow(row)
    },
    confirmExpTemSelect() {
      if (!this.currentExpTemId) {
        this.$message.warning('请选择一个实验模板')
        return
      }
      const selected = this.expTemTableList.find(item => item.temId === this.currentExpTemId)
      this.expTemId = this.currentExpTemId
      this.expTemName = selected ? selected.temName : ''
      this.expTemDialogVisible = false
      getTem(this.expTemId).then(response => {
        const stepList = response.stepList || []
        this.form.stepList = this.transformStepList(stepList)
      })
    },
    ...
    submitForm() {
      this.$refs.form.validate(valid => {
        ...
        this.form.planStartTime = this.dateRange && this.dateRange.length > 0 ? this.dateRange[0] : undefined
        this.form.planEndTime = this.dateRange && this.dateRange.length > 0 ? this.dateRange[1] : undefined
        const submitData = {
          expId: this.form.expId,
          expName: this.form.expName,
          expCode: this.form.expCode,
        ...
        if (this.form.expId) {
          updateExp(submitData).then(() => {
            this.$message.success('修改成功')
            this.visible = false
            this.$emit('refresh')
          })
        } else {
          addExp(submitData).then(() => {
